...
try {
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS exam_seating (
            ...
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            UNIQUE KEY unique_schedule_student (exam_schedule_id, student_id),
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS exam_results (
            ...
            remarks TEXT NULL,
            status ENUM('submitted', 'published') DEFAULT 'submitted',
            entered_by INT NULL,
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS exam_statistics (
            id INT AUTO_INCREMENT PRIMARY KEY,
            exam_schedule_id INT NOT NULL UNIQUE,
            total_students INT DEFAULT 0,
            present_students INT DEFAULT 0,
            absent_students INT DEFAULT 0,
            highest_marks DECIMAL(5,2) NULL,
            lowest_marks DECIMAL(5,2) NULL,
            average_marks DECIMAL(5,2) NULL,
            average_percentage DECIMAL(5,2) NULL,
            pass_count INT DEFAULT 0,
            fail_count INT DEFAULT 0,
            pass_percentage DECIMAL(5,2) DEFAULT 0,
    ...
    // Insert default grade scale if empty
    $stmt = $pdo->query("SELECT COUNT(*) FROM grade_scales");
    if ($stmt->fetchColumn() == 0) {
        $pdo->exec("
            INSERT INTO grade_scales (grade, min_percentage, max_percentage, grade_point, description) VALUES
            ('A+', 90, 100, 4.0, 'Excellent'),
            ('A', 80, 89.99, 3.7, 'Very Good'),
            ('B+', 70, 79.99, 3.3, 'Good'),
            ('B', 60, 69.99, 3.0, 'Above Average'),
            ('C+', 50, 59.99, 2.5, 'Average'),
            ('C', 40, 49.99, 2.0, 'Below Average'),
            ('D', 30, 39.99, 1.5, 'Pass'),
            ('F', 0, 29.99, 0.0, 'Fail')
        ");
    }
    ...
    elseif ($resource === 'statistics') {
        if ($method === 'GET') {
            ...
            $response = [
                ...
                'present_students' => (int)($stats['present_students'] ?? 0),
                'absent_students' => (int)($stats['absent_students'] ?? 0),
                'highest_marks' => (float)($stats['highest_marks'] ?? 0),
                'lowest_marks' => (float)($stats['lowest_marks'] ?? 0),
                'average_marks' => (float)($stats['average_marks'] ?? 0),
                'average_percentage' => (float)($stats['average_percentage'] ?? 0),
                'passed_count' => (int)($stats['pass_count'] ?? 0),
                'failed_count' => (int)($stats['fail_count'] ?? 0),
                'pass_percentage' => (float)($stats['pass_percentage'] ?? 0)
            ];
            
            echo json_encode(['success' => true, 'statistics' => $response]);
        }
    }
...
function updateExamStatistics($pdo, $scheduleId) {
    // Get pass marks for this schedule
    $stmt = $pdo->prepare("SELECT pass_marks, max_marks FROM exam_schedules WHERE id = ?");
    $stmt->execute([$scheduleId]);
    $schedule = $stmt->fetch(PDO::FETCH_ASSOC);
    $passMarks = $schedule['pass_marks'] ?? 40;
    ...
    $stmt = $pdo->prepare("
        SELECT 
            ...
            MIN(CASE WHEN marks_obtained IS NOT NULL THEN marks_obtained END) as lowest_marks,
            AVG(marks_obtained) as average_marks,
            AVG(percentage) as average_percentage,
            COUNT(CASE WHEN marks_obtained >= ? THEN 1 END) as pass_count,
            COUNT(CASE WHEN marks_obtained < ? AND marks_obtained IS NOT NULL THEN 1 END) as fail_count
        FROM exam_results
        WHERE exam_schedule_id = ?
    ");
    $stmt->execute([$passMarks, $passMarks, $scheduleId]);
    $stats = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $stmt = $pdo->prepare("
        ...
        (exam_schedule_id, total_students, present_students, absent_students, highest_marks, lowest_marks, average_marks, average_percentage, pass_count, fail_count, pass_percentage)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE
