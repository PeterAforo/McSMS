    ...
    switch ($method) {
        case 'GET':
            if ($id) {
                ...
                // Get all roles
                $stmt = $pdo->query("SELECT * FROM roles ORDER BY category, role_name");
        ...
        case 'POST':
            ...
            $stmt = $pdo->prepare("
                INSERT INTO roles (role_name, role_code, description, category, status) 
                VALUES (?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $data['role_name'],
        ...
        case 'PUT':
            ...
            $stmt = $pdo->prepare("
                UPDATE roles 
                SET role_name = ?, role_code = ?, description = ?, category = ?, status = ?
                WHERE id = ?
            ");
            $stmt->execute([
                $data['role_name'],
